...
public class DecisionTree {
	...
	public void buildDecisionTree(TreeNode node, String[][] deData, boolean flags[],
		...
		String attribute = selectAtrribute(node, transTable, flags, attributes, attrIndexMap);
		...
		if(!childs.isEmpty()) {
			for(TreeNode child:childs) {
				...
				if(Preprocess.realCols.get(attrIndex)){
					subData= generateSubdataByNum(deData, attrIndex, child.element);//deData��Ҫ��ʵֵ
				}else{
					subData= generateSubdataByString(deData, attrIndex, child.element);//deData��Ҫ��ʵֵ
				}
				...
				buildDecisionTree(child, subData, subFlags, newAttributes, attrIndexMap);
			}
		}
	}
	
	public String[][] generateSubdataByString(String[][] dedata, int attrIndex, String element){
		int rowNo=0;
		int colNo=dedata[0].length;
		for(int i=0;i<dedata.length;i++){
			if(dedata[i][attrIndex].equals(element)) rowNo++;
		}
		
		String[][] returnData=new String[rowNo][colNo];
		int returni=0;
		for(int i=0;i<dedata.length;i++){
			if(dedata[i][attrIndex].equals(element)){
	...
	public String[][] generateSubdataByNum(String[][] dedata, int attrIndex, String element){
		int rowNo=0;
		int colNo=dedata[0].length;
		double threshold = Double.valueOf(element.substring(3));
		
		for(int i=0;i<dedata.length;i++){
			double temp = Double.valueOf(dedata[i][attrIndex]);
			if(element.substring(0, 3).equals("le*") && temp <= threshold) rowNo++;
			else if(element.substring(0, 3).equals("gt*") && temp > threshold) rowNo++;
		}
		...
		for(int i=0;i<dedata.length;i++){
			double temp = Double.valueOf(dedata[i][attrIndex]);
			if(element.substring(0, 3).equals("le*") && temp <= threshold){
				for(int j=0;j<colNo;j++){
					returnData[returni][j]=dedata[i][j];
					returni++;
				}
			}else if(element.substring(0, 3).equals("gt*") && temp > threshold){
				for(int j=0;j<colNo;j++){
					returnData[returni][j]=dedata[i][j];
					returni++;
				}
